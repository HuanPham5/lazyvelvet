...
static void printUsage()
{
	...
	puts("\t-lazy_search <yes|no>\t\t: limit paired-end resolution to a lazy frontier (default: no)");
...
int main(int argc, char **argv)
{
	...
	for (arg_index = 2; arg_index < argc; arg_index++) {
		...
		if (strcmp(arg, "-cov_cutoff") == 0) {
			...
			scaffolding =
			    (strcmp(argv[arg_index], "yes") == 0);
		} else if (strcmp(arg, "-lazy_search") == 0) {
			lazySearch =
			    (strcmp(argv[arg_index], "yes") == 0);
	...
	if (expectedCoverage > 0) {
		...
		for ( ;pebbleRounds > 0; pebbleRounds--) {
			if (lazySearch)
				lazyExploitShortReadPairs(graph, sequences, dubious, shadows, scaffolding);
			else
