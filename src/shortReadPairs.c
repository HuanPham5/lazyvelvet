...
static LazyQueue *newLazyQueue(IDnum nodes)
{
	LazyQueue *queue = mallocOrExit(1, LazyQueue);

	queue->capacity = 2 * nodes + 1;
	queue->entries = mallocOrExit(queue->capacity, IDnum);
	queue->scheduled = callocOrExit(queue->capacity, boolean);
	queue->head = 0;
	queue->tail = 0;
...
static void destroyLazyQueue(LazyQueue *queue)
{
	if (queue == NULL)
		return;

	free(queue->entries);
	free(queue->scheduled);
	free(queue);
}
...
static IDnum lazyQueuePop(LazyQueue *queue)
{
	...
	nodeID = queue->entries[queue->head];
	queue->head = (queue->head + 1) % queue->capacity;

	index = nodeID + nodeCount(graph);
	if (index >= 0 && index < queue->capacity)
		queue->scheduled[index] = false;
...
void lazyExploitShortReadPairs(Graph * argGraph,
			       ReadSet * reads,
			       boolean * dubious,
			       boolean * shadows,
			       boolean force_jumps)
{
	...
	velvetLog("Starting lazy repeat resolution...\n");
	...
	localScaffold = callocOrExit(2 * nodes + 1, MiniConnection);
